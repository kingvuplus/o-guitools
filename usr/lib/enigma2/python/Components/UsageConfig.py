...
def InitUsageConfig():
    ...
    if getBrandOEM() in ('vuplus', 'ini', 'ceryon'):
    ...
    config.workaround.wakeuptimeoffset = ConfigSelection(default='standard', choices=[('-300', _('-5')),
     ('-240', _('-4')),
     ('-180', _('-3')),
     ('-120', _('-2')),
     ('-60', _('-1')),
    ...
    config.usage.leave_movieplayer_onExit = ConfigSelection(default='no', choices=[('no', _('No')), ('popup', _('With popup')), ('without popup', _('Without popup'))])
    ...
    config.usage.show_channel_jump_in_servicelist = ConfigSelection(default='alpha', choices=[('quick', _('Quick Actions')), ('alpha', _('Alpha')), ('number', _('Number'))])
